...
sudo apt-get install redis-server
 ```
* libssl required for the cirquity-multi-hashing module
  * For Ubuntu: `sudo apt-get install libssl-dev`

* Boost is required for the cirquity-cryptonote-util module
...
```bash
git clone https://github.com/cirquity/cirquity-pool.git cirquity-pool
cd cirquity-pool
...
"coinUnits": 100, // for cirquity, this is 100, 000, 000 (1 hundred million)

/* Number of coin decimals places for notifications and front-end */
"coinDecimalPlaces": 2, //for cirquity, this should be "2"
  
/* Coin network time to mine one block, see DIFFICULTY_TARGET constant in DAEMON_CODE/src/cryptonote_config.h */
"coinDifficultyTarget": 30,

/* Set daemon type. Supported values: default, forknote (Fix block height + 1), bytecoin (ByteCoin Wallet RPC API, w/ block height fix), cirquity (ByteCoin Wallet RPC API but no block height fix required)  */
"daemonType": "cirquity", 
...
"poolServer": {
    ...
    "poolAddress": "<your cirquity address here>", //note that this is the address block reqards are paid to, so should be the wallet your connected to in poolwallet
...
"wallet": {
    "host": "127.0.0.1",
    "port": 18127 /* for eg: specify any bind port you like - should math the --rpc-bind-port of your service startup command (default:8070)*/
},
...
Use this [example](https://github.com/cirquity/cirquity-pool/blob/master/deployment/cirquity-pool.service) to create the systemd service `/lib/systemd/system/cirquity-pool.service`
Then enable and start the service with the following commands : 

```
sudo systemctl enable cirquity-pool.service
sudo systemctl start cirquity-pool.service
```
